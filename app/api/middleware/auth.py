...
async def get_current_agent_for_heartbeat(
    request: Request,
    db: Session = Depends(get_db),
) -> str:
    """Heartbeat-specific auth: same as get_current_agent but does NOT require
    the agent status to be ACTIVE. This allows INACTIVE agents to call heartbeat
    and recover to ACTIVE state.
    """
    source_ip = _get_client_ip(request)
    missing_headers = [h for h in REQUIRED_HEADERS if h not in request.headers]
    if missing_headers:
    ...
    # NOTE: No Agent.status == ACTIVE check here — heartbeat is allowed for INACTIVE agents
    credential = (
        db.query(AgentCredential)
        .join(Agent)
        .filter(
...
async def get_current_admin_or_agent(
    request: Request,
    db: Session = Depends(get_db),
) -> dict:
    try:
